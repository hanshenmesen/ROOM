...
async function requestProviderConfig(request: Request): Promise<AgentProviderOverride | undefined> {
  ...
  if (config.petQaUserEmail.length > 320 || /[\r\n]/.test(config.petQaUserEmail) || (config.petQaUserEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(config.petQaUserEmail))) {
    throw new PetQaError("Pet QA 企业邮箱格式不合法。", 400);
  }
  if (config.petQaAppId.length > 128 || (config.petQaAppId && !/^[A-Za-z0-9._:-]+$/.test(config.petQaAppId))) {
    throw new PetQaError("Pet QA App ID 格式不合法。", 400);
  }
  try {
