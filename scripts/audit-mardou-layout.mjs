...
const failures = verifiedPoints.flatMap(({ name, point }) => {
  ...
  if (clearance < 1.2) reasons.push(`structure clearance ${clearance.toFixed(3)} < 1.2`);
...
console.log(`\nverified ${verifiedPoints.length} authored points with floor support and >= 1.2 structure clearance`);
