  ...
  for (let row = 0; row < accessor.count; row += 1) {
    for (let column = 0; column < itemSize; column += 1) {
      array[row * itemSize + column] = dataView[component.read](
        byteOffset + row * byteStride + column * component.bytes, true,
      );
    }
  }
...
// private route: ground -> stairs -> landing (lobby -> private) per WorldCanvas.tsx lines 194-209
const priv = [
  { name: "ground", point: [0, 1.5, -10] },
  { name: "stairs", point: [2.5, 2.5, -12] },
  { name: "landing", point: [2.5, 4.8, -15] },
  { name: "private focus camera", point: [0, 4.8, -16] },
];
console.log("--- private route segments (lobby -> private) ---");
for (let i = 0; i < priv.length - 1; i += 1) {
  const a = priv[i].point, b = priv[i+1].point;
  console.log(`segment ${priv[i].name} -> ${priv[i+1].name}`);
  for (let t = 0; t <= 1; t += 0.1) {
    const p = [a[0]+(b[0]-a[0])*t, a[1]+(b[1]-a[1])*t, a[2]+(b[2]-a[2])*t];
    const c = horizontalClearance(...p);
    const flag = Number.isFinite(c) && c < 0.35 ? " <-- BELOW 0.35m" : Number.isFinite(c) && c < 1.2 ? " <-- below 1.2m audit threshold" : "";
    console.log(`  t=${t.toFixed(1)} p=${p.map(v=>v.toFixed(2))} clearance=${Number.isFinite(c)?c.toFixed(3):"inf"}${flag}`);
  }
}
