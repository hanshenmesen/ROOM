...
test("browser Agent config ships with the DeepSeek defaults", () => {
  ...
  // The optional concurrent website slot defaults to Qwen 3.5 over the
  // public DashScope Anthropic-compatible endpoint.
  assert.equal(DEFAULT_BROWSER_AGENT_CONFIG.website.baseUrl, "https://dashscope.aliyuncs.com/apps/anthropic");
  assert.equal(DEFAULT_BROWSER_AGENT_CONFIG.website.model, "qwen3.5-plus");
...
test("tracked presets are public only; runtime presets resolve ids from base urls and models", () => {
  assert.equal(BROWSER_AGENT_PROVIDER_PRESETS.length, 1);
  assert.deepEqual(BROWSER_AGENT_PROVIDER_PRESETS.map((preset) => preset.id), ["deepseek"]);
  ...
  assert.equal(browserAgentProviderPresetId({ baseUrl: "https://custom-provider.example/anthropic", model: "custom-model" }, ALL_PRESETS), CUSTOM_BROWSER_AGENT_PROVIDER_ID);
...
test("browser Agent config crosses the parsing boundary without using a URL query", () => {
  const headers = new Headers(browserAgentConfigHeaders({
    maas: {
      ...DEFAULT_BROWSER_AGENT_CONFIG.maas,
    ...
    website: {
      ...DEFAULT_BROWSER_AGENT_CONFIG.website,
    ...
    petQa: {
      ...DEFAULT_BROWSER_AGENT_CONFIG.petQa,
  ...
  assert.equal(parsed?.maasProtocol, "anthropic");
  assert.equal(parsed?.maasAuthMode, "bearer");
});
...
test("an internal gateway's user email crosses the parsing boundary alongside the key", () => {
  ...
  assert.equal(parsed?.maasProtocol, "openai");
  assert.equal(parsed?.maasAuthMode, "api-key");
  assert.equal(parsed?.maasAppId, "visitor-app");
});
...
test("browser pet QA config crosses only the pet request boundary", () => {
  ...
  assert.deepEqual(parsed, {
    ...
    petQaMode: "tool",
    petQaUserEmail: "",
    petQaProtocol: "anthropic",
    petQaAuthMode: "bearer",
    petQaAppId: "",
  });
...
test("a stored config missing userEmail (pre-internal-maas schema) normalizes to an empty string", () => {
  const normalized = normalizeBrowserAgentConfig({
