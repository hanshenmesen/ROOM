...
function openAiToolResponse(toolName: string, args: unknown, usage = { prompt_tokens: 100, completion_tokens: 50 }) {
  return Response.json({
    choices: [{
      message: {
        role: "assistant",
        // The real internal-maas gateway sends an empty string here on a
...
test("profile extraction on the internal-maas gateway calls /v1/chat/completions with the internal header set", async () => {
...
test("the website planner on the internal-maas gateway uses OpenAI function calling", async () => {
  const originalFetch = globalThis.fetch;
  const tracer = createAgentTracer(`internal-maas-planner-${crypto.randomUUID()}`);
  const requests: Array<{ url: string; headers: Headers; body: Record<string, unknown> }> = [];
...
test("pet QA on the internal-maas gateway uses OpenAI function calling with the internal header set", async () => {
  const originalFetch = globalThis.fetch;
