...
test("provider config uses documented defaults without claiming readiness", () => {
  ...
  assert.equal(DEFAULT_WEBSITE_AGENT_BASE_URL, "https://dashscope.aliyuncs.com/apps/anthropic");
  assert.equal(DEFAULT_WEBSITE_AGENT_MODEL, "qwen3.5-plus");
...
test("a browser override never mixes with server-side provider keys", () => {
  ...
  process.env.MAAS_USER_EMAIL = "operator@example.com";
  const config = getAgentProviderConfig({
  ...
  assert.equal(config.maas.userEmail, "");
});

test("a visitor can explicitly configure an OpenAI-compatible gateway", () => {
  clearAgentEnvironment();
  const config = getAgentProviderConfig({
    maasApiKey: "visitor-key",
    maasBaseUrl: "https://gateway.example/v1",
    maasModel: "custom-model",
    maasProtocol: "openai",
    maasAuthMode: "bearer",
    maasUserEmail: "visitor@example.com",
    maasAppId: "visitor-app",
  });

  assert.equal(config.maas.protocol, "openai");
  assert.equal(config.maas.mode, "tool");
  assert.equal(config.maas.authMode, "bearer");
  assert.equal(config.maas.userEmail, "visitor@example.com");
  assert.equal(config.maas.appId, "visitor-app");
});
