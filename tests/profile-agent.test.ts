    ...
    const profile = await extractProfileFromAttachmentWithAgent(
      ...
      "",
      {
        providerConfig: {
          ...
          maasBaseUrl: "https://custom-provider.example/v1",
          maasModel: "custom-claude-model",
          maasMode: "tool",
        },
      },
    );
...
test("profile Agent supports a custom tool-compatible provider", async () => {
  const originalFetch = globalThis.fetch;
  let calls = 0;
  globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    assert.equal(String(input), "https://custom-provider.example/v1/messages");
    const body = JSON.parse(String(init?.body)) as {
      model: string;
      tools?: Array<{ input_schema: { properties: Record<string, unknown> } }>;
      tool_choice?: { type: string };
