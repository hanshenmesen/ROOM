...
const INTERNAL_MAAS_APP_ID = process.env.INTERNAL_MAAS_APP_ID;
...
test("isInternalMaasGatewayProvider matches only the internal gateway host", () => {
  assert.equal(isInternalMaasGatewayProvider("https://internal-maas.example"), true);
  assert.equal(isInternalMaasGatewayProvider("internal-maas.example"), true);
  assert.equal(isInternalMaasGatewayProvider("https://internal-maas.example/v1"), true);
  assert.equal(isInternalMaasGatewayProvider("https://api.deepseek.com/anthropic"), false);
  assert.equal(isInternalMaasGatewayProvider("https://external-maas.example/hackson"), false);
  assert.equal(isInternalMaasGatewayProvider("not a url"), false);
});

test("providerProtocolForBaseUrl routes only the internal-maas host to the openai protocol", () => {
  assert.equal(providerProtocolForBaseUrl("https://internal-maas.example"), "internal-maas");
...
test("buildToolCallRequest builds the internal-maas OpenAI Chat Completions request", () => {
  ...
  assert.equal(request.headers["x-maas-app-id"], INTERNAL_MAAS_APP_ID);
...
test("buildToolCallRequest disables thinking on the internal-maas protocol when asked", () => {
  const request = buildToolCallRequest({
    protocol: "internal-maas",
    baseUrl: "https://internal-maas.example",
    apiKey: "sk-internal-test-key",
...
test("buildToolCallRequest builds a visitor OpenAI request without duplicating /v1", () => {
  const request = buildToolCallRequest({
    protocol: "openai",
  ...
  assert.equal(request.headers.authorization, "Bearer visitor-key");
  assert.equal(request.headers["api-key"], undefined);
  assert.equal(request.headers["x-maas-user-email"], "visitor@example.com");
  assert.equal(request.headers["x-maas-app-id"], "visitor-app");
  assert.deepEqual(request.body.tool_choice, { type: "function", function: { name: "submit_answer" } });
});

test("visitor OpenAI api-key auth omits optional gateway headers when blank", () => {
  const request = buildToolCallRequest({
    protocol: "openai",
  ...
  assert.equal(request.url, "https://gateway.example/v1/chat/completions");
  assert.equal(request.headers["api-key"], "visitor-key");
  assert.equal(request.headers.authorization, undefined);
  assert.equal(request.headers["x-maas-user-email"], undefined);
  assert.equal(request.headers["x-maas-app-id"], undefined);
});
...
test("buildToolCallRequest refuses PDF document blocks on the internal-maas protocol", () => {
  assert.throws(
    () => buildToolCallRequest({
      protocol: "internal-maas",
      baseUrl: "https://internal-maas.example",
