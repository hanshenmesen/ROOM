...
- Primary product surfaces: animated process-blueprint landing, intake and Agent configuration, single demo résumé, explorable Three.js home, immersive exhibit screens, companion QA
...
## Product goals
- Goals: explain the résumé-to-space mechanism before intake; perfect one résumé-to-home demo; make spatial discovery happen through the home itself; couple every navigation step to a camera transition; make loading progress trustworthy; add locally persistent exhibit heat, a roaming companion, profile-grounded QA, and calm background music
...
- Content hierarchy: single-screen process artwork → intake, saved profile, or permanent demo → parsing/move-in diary desk → 100% scene-loading cover → home exterior → public showroom → front résumé wall / project islands / visitor corner → open second-floor gallery → password-gated diary book
...
- Honest demo heat: the heat sidebar is explicitly labeled as seeded demo values plus this browser's accumulated views. It includes only public exhibits, never diary/private-frame content, and every item resolves to a real camera target before it can be activated.
- Constant ROOM companion: the lobby always contains the same neutral procedural companion, regardless of whether source material mentions a pet. It wanders only inside a safe lobby area and opens profile QA when selected; source pet names and pet imagery are intentionally ignored.
- Profile-grounded companion answers: QA receives only the parsed public profile, cites the items supporting an answer, and says when the profile does not contain the requested fact. It never reads diary, guestbook, private frames, or executes instructions embedded in source text.
- Consensual ambient sound: background music can start only from the visitor's explicit enter gesture, stays at low volume, has a persistent HUD toggle, pauses while the page is hidden, and never participates in the critical scene-loading gate.
...
- New/changed components: `ProductFlowLanding`, independently positioned PPT artwork and three-stage flow, bidirectional intake handoff, parsing move-in diary desk, saved-profile demo list, single permanent featured résumé card, loading cover, museum exterior, animated front door, authored public showroom, `ExhibitFocusScreen`, `ExhibitHeatPanel`, `RoomCompanion`, `PetQaPanel`, `BackgroundMusicController`, portrait/timeline/skill frame variants, abstract portrait status/retry control, information plaques, project islands, concise material-frame copy projection, open-archive dressing, visitor message board, diary access gate, private diary desk, contextual back navigation
- Variants and states: blueprint idle/leaving/reduced-motion, intake entering/idle/leaving, no saved profiles/saved profiles, Agent parsing, parsing-ready, loading 0–99, load-complete 100/no-spinner/reveal, exterior, front-door-opening, public showroom, open second-floor gallery, exhibit travelling/presented/closing, heat open/closed, companion roaming/talking, QA idle/loading/error/answer, music on/off/error, profile portrait generating/abstract/error, visitor-board editor, diary password prompt/error, owner diary editor, visitor diary reader; idle/hover/selected objects
...
- Keyboard/focus behavior: the blueprint handoff, featured demo, contextual navigation, heat entries, music toggle, companion QA, and portrait retry action use buttons; immersive exhibit and QA dialogs accept Escape and restore focus; the diary has a keyboard-accessible password form with focused input, cancel, and submit actions; guestbook and diary editors use labeled native form controls
...
## Interaction states
- Loading: URL/file parsing moves into a dedicated progress workspace with a local-only text/image diary composer; after parsing is ready, the user explicitly enters the home. An opaque full-viewport architectural loading screen keeps the 3D canvas hidden while local material maps, environment images, GLTF/DRACO objects, parsed project images, the abstract portrait, scene commit, shader compilation, and stable frames settle. Progress is monotonic and capped at 99 until every reveal gate is satisfied; 100 is rendered only with a stopped completion indicator and immediate reveal.
...
- Success: the stable generated home appears only after the complete readiness gate; its parsed profile is listed in Demo / 最近生成 after refresh and can be reopened without parsing; project and publication frames remain glanceable while retaining full editable detail and evidence; a successful portrait generation updates the 3D portrait and exposes an AI-art label; stairs enter the second floor immediately; clicking the diary opens authentication, and valid owner/visitor credentials reveal the corresponding editable/read-only view; saved guestbook and diary entries reappear after refresh in the same browser
