...
- A cross-run Agent metrics endpoint (`/api/agent-runs/metrics`) with run counts, mean/p50/p95 latencies, measured Token coverage, estimated cost, planner fallback rate, and concurrency lease counters, plus NDJSON event export and a collapsible in-app fleet panel with a one-click window reset (`POST /api/agent-runs/metrics/reset`).
